...
var OAuthAdapterNew = function(pConsumerSecret, pConsumerKey, pSignatureMethod) {
    ...
    this.getStoredAccessToken = function() {
        return accessToken;
    };
    this.getStoredAccessTokenSecret = function() {
    ...
    this.setupWindow = function(childContent) {
        ...
        var closeAuthorize = Ti.UI.createButton({
            text: 'X', top: 0, right: 0, width: 20, height: 20
        });
        var self = this;
        window.add(closeAuthorize);
        closeAuthorize.addEventListener('click', function() {
            destroyAuthorizeUI();
        });
    };
